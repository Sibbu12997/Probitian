...
## 🏛 Documentation Architecture & Source of Truth

The ProBitian documentation follows a strict source-of-truth build pipeline:

```
/docs/*.md (Authoritative Source of Truth)
   │
   └──> node generate_docs.js (Build Process: npm run docs:generate)
           │
           ├──> /public/docs/*.md (Generated markdown with explicit build notice headers)
           └──> /public/docs/*.pdf (Generated branded PDF documents for downloads)
```

- **Authoritative Source of Truth**: All primary documentation is authored and maintained exclusively in `/docs/`. Developers must make all edits in `/docs/`.
- **Generated Public Artifacts**: Files in `/public/docs/` (`.md` and `.pdf`) are build artifacts generated by `generate_docs.js` for website downloads and public in-browser documentation viewers. Developers must **never** manually edit files in `/public/docs/`.
- **Synchronization**: Run `npm run docs:generate` whenever files in `/docs/` are updated to recompile public markdown and PDF copies.
